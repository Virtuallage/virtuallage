...
@ManagedBean(name = "transactionUpdateBean")
@ViewScoped
public class TransactionUpdateBean extends CommonBean implements Serializable{
	...
	private List<SelectItem> transactionStatusList;
	private List<SelectItem> paymentMethodList;
	private List<SelectItem> bankList;
	
	public TransactionUpdateBean() {
	...
	public void initialize(){
		...
		try{
			...
			transactionStatusList =  CodeUtil.getCodes("TS");
		}catch(Exception ex){
			ex.printStackTrace();
		}
	}
	...
	public void onSubmitAction(ActionEvent actionEvent) {	 
		...
		int res = txnAmount.compareTo(getSelectedDto().getAccount().getAccountBalance());
		
		if (res > 0) { // amount more than total outstanding
	...
	private void generateReversalForBilling(){
		...
		ProgressiveBillingService pbService = (ProgressiveBillingService)SpringBeanUtil.lookup(ProgressiveBillingService.class.getName());
		success = pbService.processTransactionUpdate(getSelectedDto(), txnAmount, selectedInvoice);
...
//		this.selectedInvoice = getSelectedDto().getTransaction().getInvoiceNo();
	...
	public List<SelectItem> getTransactionStatusList() {
		return transactionStatusList;
	}

	public void setTransactionStatusList(List<SelectItem> transactionStatusList) {
